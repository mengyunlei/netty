...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    // 表示当前NioEventLoop 的 selector 就绪事件 集合。
    ...
    NioEventLoop(NioEventLoopGroup parent, Executor executor, SelectorProvider selectorProvider,
        ...
        // 下面这三行代码 ，创建出来了 seletor 实例，也就是说 每个 NioEventLoop 都持有一个 seletor 实例。
            ...
            try {
                ...
                try {
                    ...
                    case SelectStrategy.SELECT:
                        ...
                        // 条件成立：说明 咱们 EventLoop 内没有 需要周期性 执行的任务。
                ...
                // 条件成立：当前NioEventLoop 内的 selector 上有就绪的事件。
                    ...
                    try {
                        ...
                        // ioTime * (100 - ioRatio) / ioRatio 根据 IO处理事件，计算出 一个 执行本地队列任务的 最大 时间。
                        ranTasks = runAllTasks(ioTime * (100 - ioRatio) / ioRatio);
                    }


                }
                // 条件成立： 当前NioEventLoop 内的 selector 上没有就绪的事件，只处理本地任务就可以了。
                else {
                ...
                if (ranTasks || strategy > 0) {
                    ...
                    // 正常 NioEventLoop 线程 从selector多路复用器上 唤醒后 工作是因为 有 io事件。
                    // 会把 selectCnt 置为0.
                    selectCnt = 0;
    ...
    private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
        ...
        try {
            ...
            // readyOps == 0 ？ 它是什么情况呢？ 这里会通过unsafe.read() 再次将 当前Channel 的 事件列表设置为 监听读 。
    ...
    private int select(long deadlineNanos) throws IOException {
        ...
        // deadlineNanos + 995000L?  回头谁明白了 告诉我。
